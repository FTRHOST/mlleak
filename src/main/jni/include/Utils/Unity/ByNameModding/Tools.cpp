...
void Tools::Hook(void *target, void *replace, void **backup) {
    if (!target) {
        __android_log_print(ANDROID_LOG_ERROR, "StealthMod", "HOOK FAILED: Target address is NULL!");
        return;
    }
    ...
    if (mprotect(p, (size_t) size, PROT_EXEC | PROT_READ | PROT_WRITE) == 0) {
        ...
        __android_log_print(ANDROID_LOG_INFO, "StealthMod", "HOOK SUCCESS: %p -> %p", target, replace);
    } else {
        __android_log_print(ANDROID_LOG_ERROR, "StealthMod", "HOOK FAILED: mprotect failed at %p", target);
    }
}
